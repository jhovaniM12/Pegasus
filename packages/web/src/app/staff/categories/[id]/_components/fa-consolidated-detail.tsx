...
type FaConsolidatedDetailProps = {
  ...
  onBack?: () => void;
  /**
   * Incrusta solo las selecciones por juez (sin tarjeta contenedora ni bloque
   * de finalistas). Útil cuando el consolidado ya vive en FaClosedState.
   */
  embedded?: boolean;
};

function JudgeFormsGrid({
  judgeForms,
  judgingStartedAt,
}: {
  judgeForms: ManagementJudgeForm[];
...
export function FaConsolidatedDetail({
  ...
  if (embedded) {
    if (judgeForms.length === 0) return null;

    return (
      <div className="space-y-3">
        <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Selecciones por juez</p>
        <JudgeFormsGrid judgeForms={judgeForms} judgingStartedAt={judgingStartedAt} />
      </div>
    );
  }
